...
for subdir, dirs, files in os.walk(path):
    for file in files:
        if 'EDM-1.csv' in file:
            subdir2 = subdir.replace('/', os.sep)
            path2 = path.replace('/', os.sep)
            sub_path = subdir2.replace(path2 + os.sep, '')
            sub_path = sub_path.replace('-', ' ')
            sub_path = sub_path.replace(os.sep, ' ')
            sub_path_list = sub_path.split(' ')
            sub_path_list = list(filter(None, sub_path_list))
            sub_path_list = sub_path_list[-2:]
            x = 'SS'
            if 'M' in sub_path_list[0]:
                x += sub_path_list[0]
                x += str(int(sub_path_list[1]))
            if 'line' in sub_path_list[0] or sub_path_list[0] == 'B':
                x += sub_path_list[0][0].upper()
                x += str(int(sub_path_list[1]))
            if sub_path_list[0].isdecimal():
                if 'M' in sub_path_list[1]:
                    x += sub_path_list[1]
                    x += str(int(sub_path_list[0]))
                if 'line' in sub_path_list[1] or sub_path_list[1] == 'B':
                    x += sub_path_list[1][0].upper()
                    x += str(int(sub_path_list[0]))

            # for string in sub_path_list:
            #     if string[:2] != 'up':
            ...
            #             x += string[0].upper()
            sub_folder = x
            # sub_folder = subdir2.split(os.sep)[-1]

            print("\nfolder: " + sub_folder)
            file_path = subdir
            # file_path = subdir + os.sep + file
            data_file = pd.read_csv(os.path.join(subdir, file), header=None, low_memory=False)
            ...
            # Deleting false coordinates
            if data3D0[1].dtypes == object:
                data3D0 = data3D0[~data3D0[1].str.contains('IND')]
                data3D0 = data3D0.astype(np.float64)
            ...
            for th_idx, threshold in enumerate(thresholds):
                ...
                for i, DCM in dataDCM.items():
                    ...
                    result2[i] = pd.DataFrame(
                        {"RMS" + sign: [], "Max Dev" + sign: [], "Area" + sign: [], "Normal x" + sign: [],
                         "Normal y" + sign: [], "Normal z" + sign: [], "Location" + sign: []})
                    ...
                    if len(arrayDCM) > 50:
                        ...
                        triangles_small_n = cluster_n_triangles0[triangle_clusters0] < 50  # testing minimum number of triangles
                ...
                # plot torso points
                ax.scatter(-tpts[:, 0], tpts[:, 2], tpts[:, 1], s=0.005, c='grey', alpha=0.9)
                ...
                for i, center in centroid2.items():
                    if center:
                        ...
                        for j, patch in enumerate(ccmp2[i]):
                            ...
                            ax.scatter(-patch[:, 0], patch[:, 2], patch[:, 1], s=2, alpha=0.5, label='patch ' + str(j + 1))
